      ...
      <div className="flex flex-wrap gap-4 mt-4">
        {imagePreview.map((image, index) => (
          <div key={index} className="w-24 h-24">
            <img src={image} alt={`Preview ${index}`} className='rounded-lg object-cover w-full h-full' />
          </div>
        ))}
      </div>
      {imageList&& <div className="flex flex-wrap gap-4 mt-4">
        {imageList.map((image, index) => (
          <div key={index} className="w-24 h-24">
            <img src={image?.url} alt={`Preview ${index}`} className='rounded-lg object-cover w-full h-full' />
          </div>
        ))}
      </div>}
    </div>
